...
>
> **Windows:** setup verifies connectivity after writing setup artifacts. If verification fails, setup exits non-zero and prints manual recovery steps (load extension, confirm extension ID, rerun setup with explicit ID if needed, then `tabctl ping`).
