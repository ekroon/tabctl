...
async function detectGitHubState(tabId) {
    try {
...
async function analyzeTabs(params) {
    ...
    for (const tab of tabs) {
        ...
        if (isGitHubIssueOrPr(tab.url) && isScriptableUrl(tab.url)) {
            const state = await detectGitHubState(tab.tabId);
