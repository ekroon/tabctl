...
test("openTabs throws on ambiguous group title", async () => {
  installChromeStub();
  const snapshot = makeSnapshot([
    {
      windowId: 1,
      ...
      tabs: [
        ...
        { tabId: 2, index: 1, groupId: 60, url: "https://b.com" },
      ],
      groups: [
        { groupId: 50, title: "Work" },
        { groupId: 60, title: "Work" },
      ],
    },
  ]);
  const deps = makeDeps(snapshot);
  ...
  await assert.rejects(
    () => openTabs({ urls: ["https://c.com"], groupTitle: "Work" }, deps),
    { message: /Ambiguous group title "Work"/ },
  );
});
