...
- Rust CLI MVP added: `rust/tabctl-rust-cli-readonly/`
  - Implements `version` locally with Node-provided version metadata passthrough.
  - Implements host-mediated `ping` and `history` over the existing socket protocol.
  - For core mutating commands, uses a safe passthrough back to the Node CLI to preserve behavior while Rust coverage expands.
  - Is now the default CLI path when a Rust binary is available, with explicit rollback via `TABCTL_CLI_IMPL=node`.
